...
public class GamePanel extends JPanel implements ActionListener { //Membuat class publik yang bisa diakses oleh class lain dengan nama Gamepanel yang diextend dari class JPanel ddan mengimplementasikan interface ActionListener
	...
	int lineLength = 480;					//Membuat variable int lineLength (panjang garis) yang bernilai 270 digunakan untuk garis papan tic tac toe dibawah nanti
	...
	public GamePanel() { //Constructor dari kelas GamePanel.
		Dimension size = new Dimension(650, 500); //Konfigurasi ukuran objek 'Dimension' dengan lebar 420 pixels dan tinggi 300 pixels
	...
	public void drawBoard(Graphics page) {
		...
		page.fillRoundRect(x, y + offset + offset, lineLength, lineWidth, 5, 30);
		page.fillRoundRect(x, y + offset + offset + offset, lineLength, lineWidth, 5, 30);
		...
		page.fillRoundRect(y + offset + offset, x, lineWidth, lineLength, 30, 5);
		page.fillRoundRect(y + offset + offset + offset, x, lineWidth, lineLength, 30, 5);
	}

	public void drawUI(Graphics page) {//metode buat set color dan font
		page.setColor(darkgray);// Mengatur warna latar belakang
		page.fillRect(480, 0, 200, 500); // Menggambar latar belakang untuk UI
		...
		page.drawString("Win Count", 520, 30);// Menggambar teks "Win Count"
		page.drawString(": " + player1wins, 585, 70);// Menggambar jumlah kemenangan pemain X
		page.drawString(": " + player2wins, 585, 105);// Menggambar jumlah kemenangan pemain O
...
>>>>>>> f487b5c8746e7b569a3256dd01342264e4eaeb65
		...
		if (gameDone) {
			if (winner == 1) { // x // Jika pemain X menang
				page.drawString("Pemenangnya", 515, 150);// Jika permainan sudah selesai
				page.drawImage(xImg, 545, 160, null); // Menggambar simbol X
			} else if (winner == 2) { // o
				page.drawString("Pemenangnya", 515, 150);// Menampilkan teks "Pemenangnya"
				page.drawImage(oImg, 545, 160, null); //Menggambar simbol O
			} else if (winner == 3) { // tie  // Jika permainan seri
				page.drawString("Seri", 555, 250);// Menampilkan teks "Seri"
			}
			...
			page.drawString("Sekarang", 530, 150); // Menampilkan teks "Sekarang"
			if (playerX) {
				page.drawString("Giliran X", 530, 180);  // Menampilkan teks "Giliran X"
			} else {
				page.drawString("Giliran O", 530, 180); // Menampilkan teks "Giliran O"
			}
		}
		// DRAW LOGO
		Image Wayang = Toolkit.getDefaultToolkit().getImage("wayang.png"); // Membuat objek gambar logo
		page.drawImage(Wayang, 530, 400, 80, 70, this); // Menggambar logo
		...
		page.drawString("Selamat Bermain", 505, 480); // Menampilkan teks "Selamat Bermain"
	}

	public void drawGame(Graphics page) {
		// Loop melalui setiap sel pada papan permainan
		for (int i = 0; i < 5; i++) {   		// Membuat perulangan sebanyak 5 kali untuk Bergantian Bermain
			for (int j = 0; j < 5; j++) {		// Membuat Nested loop untuk perulangan kedua
				if (board[i][j] == 0) {  		// Jika nilai sel pada papan permainan adalah 0 (kosong), tidak ada gambar yang digambar
...
>>>>>>> f487b5c8746e7b569a3256dd01342264e4eaeb65
		...
		repaint();	//memanggil method repaint untuk tetap menggambar terus.
	}
	...
	public void checkWinner() {  //Function untuk mengecek pemenang / seri
		...
		int temp = -1; //Membuat variable temporary untuk menampung pemenangnya
			...
			for (int i = 0; i < 5; i++) { //Membuat kondisi loop untuk mengecek dari index i=0 ke index i=4
				if (board[i][0] == board[i][1] && board[i][1] == board[i][2]     //// Mengecek di setiap boardnya yang menjadi rows
			...
			for (int j = 0; j < 5; j++) { //Membuat kondisi loop untuk mengecek dari index i=0 ke index i=4
				if (board[0][j] == board[1][j] && board[1][j] == board[2][j]    //// Mengecek di setiap boardnya yang menjadi columns
			...
			// Check diagonals
			if (board[0][0] == board[1][1] && board[1][1] == board[2][2]     //// Mengecek di setiap boardnya yang menjadi diagonals
			...
			} else if (board[0][4] == board[1][3] && board[1][3] == board[2][2]    /// Mengecek di setiap boardnya yang menjadi diagonals
